...
// 매개변수를 LiveData 대신 MutableLiveData 형식으로 받아와도 여전히 화면 안 뜸(상관 없는 부분인 듯)
class ItemsAdapter(val items: LiveData<ArrayList<Items>>):RecyclerView.Adapter<ItemsAdapter.Holder>() {
        ...
        override fun onBindViewHolder(holder: Holder, position: Int) {
            holder.bind(items.value?.getOrNull(position))
        }
                ...
                items?.let {
                    ...
                    binding.btnHeart.setOnClickListener {
                        if(items.isLiked == true){
                            binding.btnHeart.setImageResource(R.drawable.heart_24dp_gray)
                            items.isLiked = false
                            ItemViewModel().addHeartlist(items) // 뷰모델 함수
                            Log.d("${items.name}","찜 : ${items.isLiked}")
                        }
                        else{
                            binding.btnHeart.setImageResource(R.drawable.heart_24dp_red)
                            items.isLiked = true
                            ItemViewModel().removeHeartlist(items)  // 뷰모델 함수
                            Log.d("${items.name}","찜 : ${items.isLiked}")
                        }
                    }
                }
